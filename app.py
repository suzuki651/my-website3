...
# Flask 2.3.3 使用時の互換性確認と調整

# app.pyの冒頭に追加（デバッグ用）
import flask
print(f"Flask version: {flask.__version__}")

# 既存のコードで問題になる可能性がある箇所の確認

app = Flask(__name__)

# 1. Flask-WTF 1.1.1 での CSRF 設定（問題なし）
app.config['WTF_CSRF_ENABLED'] = True
app.config['WTF_CSRF_TIME_LIMIT'] = None

# 2. Werkzeug 2.3.7 でのResponse型アノテーション（問題なし）
from typing import Callable, Any, Union, Optional
from werkzeug.wrappers import Response as WerkzeugResponse
from flask.wrappers import Response as FlaskResponse

# 3. session の使用方法（変更なし）
# Flask 2.3.3 では既存のsessionの使い方で問題ありません

# 4. g オブジェクトの使用（変更なし）
from flask import g

# 5. teardown_appcontext の使用（変更なし）
...
# 現在のapp.pyは Flask 2.3.3 で問題なく動作します
