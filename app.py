...
if __name__ == '__main__':
    ...
    # 本番環境とローカル環境の判定
    if IS_PRODUCTION:
        # 本番環境（Azure App Service）ではGunicornが使用されるため、
        # この部分は実行されません。
        logger.info("本番環境: Gunicornでアプリケーションが起動されます")
    else:
        # ローカル開発環境でのみFlaskの開発サーバーを使用
        debug_mode = True
        port = int(os.getenv('PORT', 5000))  # ローカル開発では5000番ポートを使用
        
        logger.info(f"ローカル開発環境でアプリケーション開始 - Debug: {debug_mode}, Port: {port}")
